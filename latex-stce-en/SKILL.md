---
name: latex-stce-en
description: Chuẩn hóa file LaTeX theo template STCE/HUCE Journal (Tạp chí KHCN Xây dựng HUCE). Alias gọi nhanh: /latex-stce-en, /stce-en
---
...
- Chuẩn hóa chữ hoa/thường tùy tiện sang **sentence case/normal case** ở title, section/subsection/subsubsection, caption, header bảng và text thông thường; chỉ giữ viết hoa cho tên riêng, viết tắt, ký hiệu, hoặc quy ước bắt buộc của template
...
- **KHÔNG** viết hoa tùy tiện kiểu Title Case/ALL CAPS trong caption, heading, header bảng hoặc nội dung thường; ví dụ `Finite Element Analysis` → `finite element analysis (FEA)`, `Orthogonal Quality` → `orthogonal quality`
...
- Tên `\section`, `\subsection`, `\subsubsection`, caption và header bảng dùng **sentence case/normal case**; không dùng Title Case/ALL CAPS tùy tiện
- Chỉ viết hoa tên riêng, viết tắt, ký hiệu, tên phần theo quy ước template, hoặc chữ đầu câu
...
- Không thêm dòng trống bên trong abstract, frontmatter, author/address, equation, figure, table, caption, itemize/enumerate, tabular, hoặc giữa các dòng chỉ là line-wrap của cùng một paragraph
...
- **Ký hiệu prime trong math mode**: phải viết bằng `\prime`
  - Đúng: `$x^{\prime}$`, `$x^{\prime\prime}$`, `$x^{\prime\prime\prime}$`
  - Sai: `$x^{'}$`, `$x^{''}$`, `$x'$`, `$x''$` khi đây là ký hiệu toán
  - KHÔNG đổi dấu nháy trong text/quote
