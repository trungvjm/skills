---
name: latex-stce-vn
description: Chuẩn hóa file LaTeX theo template STCE tiếng Việt (Tạp chí KHCN Xây dựng - bản tiếng Việt). Alias gọi nhanh: /latex-stce-vn, /stce-vn
---
...
- Đổi tham chiếu subfigure như `Hình~\ref{f1}(a)` → `(Hình~\ref{f1}(a))` khi dùng trong câu
...
- **KHÔNG** viết hoa tùy tiện kiểu Title Case/ALL CAPS trong caption, heading, header bảng hoặc nội dung thường; chỉ giữ viết hoa cho tên riêng, viết tắt, ký hiệu, hoặc thuật ngữ bắt buộc
...
### 3.4. Spacing
- Giữa **các paragraph** phải có **1 dòng trống** (đây là cách LaTeX nhận biết đoạn mới)
...
- **Tham chiếu subfigure**: bọc toàn bộ tham chiếu trong ngoặc đơn, ví dụ `(Hình~\ref{fN}(a))`, `(Hình~\ref{fN}(b))` — KHÔNG viết `Hình~\ref{fN}a` và KHÔNG để trần `Hình~\ref{fN}(a)` khi dùng trong câu
